...
class CompresserRLE(CodeurCA):
	...
	def __init__(self, specialChar = 254):
		""" Constructeur de la classe CompresserRLE avec un caractère spécial par défaut à 254.
		254 car pour des images, la différence entre 254 et 255 est très faible
		contrairement à 1 et 2 qui sont très différents
		Puis pour un texte, 254 est un caractère spécial qui est très peu utilisé
		"""
	...
	def demo():
		...
		for file in files:
			...
			if (imgBinaire == d):
				voulu = Colors.blue + "Le fichier a perdu du poids"
				if percentage > 1:
					voulu = Colors.red + "Le fichier a augmenté de taille"
				# Print in green
