...
def secondDiviseur(a):
	...
	for b in range(3, int(sqrt(a)) + 1, 2):
		# On renvoie b si a est divisible par b
		if a % b == 0:
			return b
	return a
...
def eDiviseurs(a):
	""" Renvoie un ensemble croissant des diviseurs positifs de a
	...
	{1, 13}
	"""
	# Initialisation de l'ensemble avec 1 et a
	r = set({1, a})
	
	# Remplissage de l'ensemble (test de tous les diviseurs de a)
	for b in range(1, int(sqrt(a)) + 1):
		# Si b divise a, on ajoute b et a/b à l'ensemble car b <= sqrt(a)
		if a % b == 0:
			r.add(b)
			r.add(a // b)
	return r
...
class ElmtZnZ(object):
	...
	def __rpow__(self, q):
		...
		ElmtZnZ(9,10)
		"""
		# Cas général de la puissance en exponentiation rapide
		result = 1
		base = q
		power = self.rep
		while power > 0:
	...
	def ordre(self):
		...
		6
		"""
		# Variable qui va contenir le nombre, à la puissance n (initialisée à 1) 
		a = self * self.rep
		n = 1

		# Tant que le nombre calculé est différent de self, on continue
		while (a.rep != self.rep):
			a *= self.rep
			n += 1
		
		# On renvoie la puissance n
		return n
