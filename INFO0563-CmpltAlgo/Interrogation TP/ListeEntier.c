...
/**
 * @return vrai si le nombre est entre 0 et 9 inclus
 * Fonction utilisé pour rajouter un espace lors des affichages
**/
int isSmallValue(int a) { return a < 10 && a >= 0; }
...
void printListValues(ListeEntier l) {
	...
	for (i = l.List[0].next, j = 1; l.List[i].next; i = l.List[i].next, j++) {
		if (j%10 == 0) {
			fprintf(stderr, "\n");
		}
		fprintf(stderr, isSmallValue(l.List[i].value) ? " %d (x%d), " : "%d (x%d), ", l.List[i].value, l.List[i].occurences);
	}
	fprintf(stderr,"%d (x%d)\n", l.List[i].value, l.List[i].occurences);
}
...
void printListStruct(ListeEntier l) {
	...
	fprintf(stderr, "\n[");
	for (i = 0; i < l.Size; i++)
		fprintf(stderr, isSmallValue(l.List[i].value) ? " %d, " : "%d, ", l.List[i].value);
	fprintf(stderr, isSmallValue(l.List[i].value) ? " %d] Values" : "%d] Values", l.List[i].value);

	// Occurences
	fprintf(stderr, "\n[");
	for (i = 0; i < l.Size; i++)
		fprintf(stderr, isSmallValue(l.List[i].occurences) ? " %d, " : "%d, ", l.List[i].occurences);
	fprintf(stderr, isSmallValue(l.List[i].occurences) ? " %d] Occurences" : "%d] Occurences", l.List[i].occurences);

	// Prochain Index
	fprintf(stderr,"\n[");
	for (i = 0; i < l.Size; i++)
		fprintf(stderr, isSmallValue(l.List[i].next) ? " %d, " : "%d, ", l.List[i].next);
	fprintf(stderr, isSmallValue(l.List[i].next) ? " %d] Next Index\n" : "%d] Next Index\n", l.List[i].next);
...
void supprimer(int k, ListeEntier *l) {
	...
	// Si la position n'est pas trouvée, afficher un message d'erreur
		...
		fprintf(stderr,"\nSuppression impossible, la position indiquée est en dehors de la liste");
}
